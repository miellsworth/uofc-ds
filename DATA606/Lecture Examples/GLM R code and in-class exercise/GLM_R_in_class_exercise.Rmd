...
BIDATA<-data.frame(explan4,pi_fit_hat,pi_fit)
...
explan4<-c(0,2,4,5)
BIDATA3<-data.frame(explan4,pi_hat,pi_fit3)
ggplot()+geom_point(data = BIDATA3, aes(explan4, pi_hat))+geom_line(data = BIDATA3, aes(explan4, pi_fit3), colour='red')
```
...
library(caret)
training.samples = createDataPartition(data$Pclass, p = 0.95, list = FALSE)
train.data = data[training.samples, ]
test.data  = data[-training.samples, ]

# Step 1: apply logistic regression to train data
fit4 <- glm(Survived ~ factor(Pclass) + factor(Sex) + Age + factor(SibSp) + factor(Parch), data = train.data, family = binomial)

# Step 2: apply the fitted model to test and get the prediction accuracy
fitted.results <- predict(fit4, newdata = test.data, type = 'response')
fitted.results <- fitted.results > 0.5
mean(fitted.results == test.data$Survived)
```
...
#newdata
# Create bins
#bin <- crabDATA %>% mutate(width_bin = cut(width, breaks = c(0, 23.25,24.25,25.25,26.25,27.25,28.25,29.25, 100)))

# Check the variance, compare it with mean
#newdata <- bin %>%
#  group_by(width_bin) %>%
#  summarise(count = n(), mean = mean(satell), variance = var(satell))
```
...
```{r}
mu_fittt<-predict(fit7, newdata = data_explan, type = "response")
```

```{r}
mu_fitt<-predict(fit6, newdata = data_explan, type="response")
scatter.smooth(bin_p,sample_mean)
lines(bin_p, mu_fitt, col="red")
...
fit8 <- glm(data = DATA, Death~offset(log(Pop))+Age_level+Smoke_status, family = poisson())
summary(fit8)
```
